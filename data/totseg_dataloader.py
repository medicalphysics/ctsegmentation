...
class TotSegDataset2D(Dataset):
    ...
    def _get_batch_part(self, idx):
        ...
        if end[0] > sh[0] or end[1] > sh[1]:
            ...
            label_part = np.pad(label.slicer[beg[0]:end_p[0], beg[1]:end_p[1], split[2]:split[2]+1].get_fdata(), pads, constant_values=0).reshape((1, 1) + self._train_shape + (1,)).astype(np.uint8)
        else:
            image_part = image.slicer[beg[0]:end[0], beg[1]:end[1], split[2]:split[2]+1].get_fdata().reshape((1, 1) + self._train_shape)
            label_part = label.slicer[beg[0]:end[0], beg[1]:end[1], split[2]:split[2]+1].get_fdata().reshape((1, 1) + self._train_shape + (1,)).astype(np.uint8)         
                
        label_t = torch.zeros((1, 1) + self._train_shape + (self.max_labels+1,), dtype=torch.bool)                
        with torch.no_grad():
            label_part_t = torch.as_tensor(label_part, dtype=torch.int64)
            label_t.scatter_(4, label_part_t, 1)
